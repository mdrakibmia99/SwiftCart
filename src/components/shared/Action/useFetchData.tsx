...
export const useFetchData = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const categoryItems: SubcategoryItem[] = data.map((cat: { _id: string; name: string }) => ({
        name: cat.name,
        id: cat._id
      }));
  ...
  const fetchBrands = async () => {
    try {
      ...
      const brandItems: SubcategoryItem[] = data.map((ba: { _id: string; name: string }) => ({
        name: ba.name,
        id: ba._id
      }));
  ...
  const fetchFeaturedProducts = async () => {
    try {
      ...
      const featuredProduct: SubcategoryItem[] = data.slice(0, 8).map((prod: { _id: string; name: string }) => ({
        name: prod.name,
        id: prod._id
      }));
  ...
  const fetchFlashsales = async () => {
    try {
      ...
      const flashes: SubcategoryItem[] = data.map((prod: { _id: string; name: string }) => ({
        name: prod.name,
        id: prod._id
      }));
      setFlash(flashes);
  ...
  const fetchTrending = async () => {
    try {
      ...
      const trendingItems: SubcategoryItem[] = data.map((prod: { _id: string; name: string }) => ({
        name: prod.name,
        id: prod._id
      }));
